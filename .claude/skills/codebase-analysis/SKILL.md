...
```bash
# Source files and app surface
rg --files src public -g '*.ts' -g '*.tsx' -g '*.css' -g '*.svg'

# Estimate size per source file
wc -l $(rg --files src -g '*.ts' -g '*.tsx' -g '*.css') 2>/dev/null | sort -rn

# Entry points and project commands
# Read README.md, package.json, vite.config.ts, tsconfig*.json, src/main.tsx, and src/App.tsx.
```
...
| Observed scope | Type |
|---------------|------|
| 1 folder, cohesive set of components/hooks/helpers | `module` |
| Multiple components/hooks/helpers around a single functional flow | `component` |
| Multiple folders coordinated toward a function | `subsystem` |
| Sudoku, Archipelago, or configuration vocabulary boundary | `bounded-context` |
...
# Use Glob for docs/ai/analysis-plan-*.md, then Read the matching plan files.

# Units already documented in project memory
# Read docs/ai/architecture-map.md if it exists.
```
...
───────────────────────────
Estimated size: ~XXk lines, ~XX source files

Identified units:

  [HIGH]   <name>   [<type|?>]   ~XX files   <path>
  [HIGH]   <name>   [<type|?>]   ~XX files   <path>
  [MEDIUM] <name>   [<type|?>]   ~XX files   <path>
  [LOW]    <name>   [<type|?>]   ~XX files   <path>
...
Phase 1 is done entirely by search. Feeling the need to read a file
during Phase 1 = signal that we are already in Phase 2.

**Rule 2 — Surgical reading (Phases 2-5)**
- Search first, read only if search results are insufficient
...
```bash
# 1.1 PIVOT — files imported by the unit and likely starting points
# → starting point for Phase 2 and Phase 3
rg -n "^import .*from ['\"]" src/<unit> --glob '*.{ts,tsx}'

# 1.2 ENTRY — React/browser entry points and external callers
# → starting point for Phase 4 (integration mapping)
rg -n "createRoot|<App|on[A-Z][A-Za-z]+=|useEffect|addEventListener" src --glob '*.{ts,tsx}'

# 1.3 OUTPUT — external package and integration dependencies
# → archipelago.js, React, browser APIs, assets, and other third-party surfaces
rg -n "^import .*from ['\"][^./]|archipelago|Client|fetch\(|WebSocket|localStorage|sessionStorage|import.meta.env" src package.json --glob '*.{ts,tsx,json}'

# 1.4 SHAPE — exported components, hooks, functions, interfaces, and app state
# → identifies component boundaries, helpers, hooks, and local contracts
rg -n "export (default )?(function|const|class|interface|type)|function [A-Z]|const [A-Z][A-Za-z0-9]+\s*=\s*\(|useState|useReducer" src --glob '*.{ts,tsx}'
```
...
```
ORIENTATION MAP — <unit>
─────────────────────────────
Pivot            : <file/component/hook> (X references or imports)
External callers : <entry/component/hook1>, <entry/component/hook2>
Key dependencies : <package/API/client1>, <package/API/client2>
Shape            : <component> -> <hooks/helpers/state>
Pattern          : <global> + <local pattern if different>

Phase 2 starting point: <pivot file/component>
Phase 4 starting point: <entry point or user-triggered flow>
```

### Phase 2 — Domain Model Extraction

1. Locate model and state definitions based on the pattern identified in Phase 1:
   - React app   : component props, hook state, derived values, and event handlers
   - Game logic  : puzzle/grid/cell/config types, generators, validators, and reducers
   - Integration : Archipelago client/session/item/location types and message handlers
   - Unknown     : look for exported `type`/`interface` definitions and pure helpers with no React dependency
...
- List the entry points that mutate the affected state (components, hooks, event handlers, integration callbacks)
- Check each one for the rule
- Any entry point missing the rule → **coverage gap** (critical finding)

```bash
# Find all mutation paths for the affected state/model
rg -n "set[A-Z]|useState|useReducer|dispatch|localStorage|sessionStorage|client\.|connect|send|receive|check|locations" src/<unit> --glob '*.{ts,tsx}'
```
...
BUSINESS LOGIC — <unit>
───────────────────────────────────────────────────
Entry points covered  : <UI handler X [File:line], effect or Archipelago callback Y [File:line]>
Main flow             : <methodA> → <methodB>  [File:line]
                          ? <call or type not followed>  — <reason>  [File:line]
                        → <methodC>  [File:line]
Rules identified      :
...
1. Find all entry points exposed by the unit:
   ...
   ```bash
   # React and browser entry points (IN)
   rg -n "createRoot|<App|on[A-Z][A-Za-z]+=|useEffect|addEventListener" src --glob '*.{ts,tsx}'

   # User-visible configuration and app wiring (IN)
   rg -n "props|useState|useReducer|import.meta.env|URLSearchParams" src/<unit> --glob '*.{ts,tsx}'
   ```

2. Find all outgoing integration calls:

   ```bash
   # Archipelago and WebSocket/client calls
   rg -n "archipelago|Client|connect|disconnect|socket|WebSocket|items|locations|DeathLink" src package.json --glob '*.{ts,tsx,json}'

   # HTTP, browser storage, and environment configuration
   rg -n "fetch\(|localStorage|sessionStorage|navigator\.|window\.|document\.|import.meta.env" src/<unit> --glob '*.{ts,tsx}'

   # Assets and styling dependencies
   rg -n "import .*\.css|import .*\.svg|public/|url\(" src public --glob '*.{ts,tsx,css,svg}'
   ```
   ...
   rg -n "class .* implements <InterfaceName>|: <InterfaceName>|satisfies <InterfaceName>" src --glob '*.{ts,tsx}'
   ```
...
4. **Detect implicit coupling violations** — based on the architectural pattern
   ...
   | React | pure game/domain helpers must not import React components or CSS |

   Run the search corresponding to the detected pattern. Any result = architectural
...
5. **Map outgoing triggers** — what features/flows does this unit activate as output,
   ...
   ```bash
   rg -n "publish|emit|trigger|notify|dispatch|send|receive|subscribe|useEffect|setTimeout|setInterval|client\." src/<unit> --glob '*.{ts,tsx}'
   ```
...
covered by the search patterns → investigate manually before closing Phase 4.
...
1. **Cross-reference phases 3 and 4 first (no new reads needed)**
   ...
   **Phase 3 `[IMPLICIT]` items — validate before promoting:**
   For each `[IMPLICIT]` from Phase 3, attempt confirmation with Phase 5 searches:
...
2. **Targeted greps**
   ...
   ```bash
   # Explicit thrown errors and promise/callback failures
   rg -n "throw new|catch \(|\.catch\(|console\.(error|warn)" src/<unit> --glob '*.{ts,tsx}'

   # Shared mutable state, timers, subscriptions, and side effects
   rg -n "let .* =|static |useEffect|setInterval|setTimeout|addEventListener|removeEventListener|client\.|connect\(" src/<unit> --glob '*.{ts,tsx}'

   # Browser persistence and external session boundaries
   rg -n "localStorage|sessionStorage|indexedDB|archipelago|Client|WebSocket|import.meta.env" src/<unit> package.json --glob '*.{ts,tsx,json}'
   ```

3. **Tests — method names only (edge cases documented by the team)**

   ```bash
   rg -n "describe\(|it\(|test\(" src test --glob '*.{test,spec}.{ts,tsx}'
   ```

4. **Git history — past incidents**

   ```bash
   git log --oneline --all -- src/<unit>
   ```
...
Flow : <feature name>
──────────────────────────────────────
Entry : <type (UI / EFFECT / ARCHIPELAGO / BROWSER)> — <trigger>
...
the unit name in file names, exports, and imports to find a natural anchor:
```bash
rg -n "<name>" src --glob '*.{ts,tsx}' -l
```
