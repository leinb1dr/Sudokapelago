...
- "Archipelago location checks must be idempotent"
  → .claude/rules/archipelago.md  [high — constraint stated explicitly in analysis]
- "Never call the external API within a transaction"
  → .claude/rules/archipelago.md  [high — pattern grep-confirmed in 3 source files]
