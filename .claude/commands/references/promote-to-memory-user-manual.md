...
```
PROMOTE-CANDIDATE:
- "Archipelago location checks must be idempotent" → .claude/rules/archipelago.md
...
```
Decision: do not send Archipelago location checks from render-time code.
→ Promoted to: .claude/rules/archipelago.md
```
